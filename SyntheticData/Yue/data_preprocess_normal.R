...
bindori_dataset_threshold_chr <- bindori_dataset_threshold %>% mutate(across(c(where(is.numeric), -weight), as.factor))
print("factor done!!!")
str(bindori_dataset_threshold_chr)
bindori_dataset_threshold_chr[,87:90] <- lapply(bindori_dataset_threshold_chr[,87:90], factor)
str(bindori_dataset_threshold_chr)
# preprocessed original data import and export
#-----------------------------------------------
export_path <- "/Users/roxy/Desktop/Master-Thesis-SyntheticDataGeneration"
bindori_data_name <- "bindori_dataset_preprocessed_normal.rda"
