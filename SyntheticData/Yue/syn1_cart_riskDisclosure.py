...
ods_dataset = pyreadr.read_r("bindori_dataset_preprocessed_factor.rda")[
    "bindori_dataset_threshold_chr"]
# load the cart synthetic datasets
cart_sample_sds = pyreadr.read_r(
    "./SyntheticData/Yue/syn1_cart/cart_sample_syn.rda")["syn"]
cart_norm_sds = pyreadr.read_r(
    "./SyntheticData/Yue/syn1_cart/cart_norm_syn.rda")["syn"]
cart_normrank_sds = pyreadr.read_r(
    "./SyntheticData/Yue/syn1_cart/cart_normrank_syn.rda")["syn"]
...
def load_ods_preprocess():
    ...
    ods_dataset[ods_dataset.columns[1:54]
                ] = ods_dataset[ods_dataset.columns[1:54]].astype(float)
...
def load_sds_cart_preprocess():
    ...
    # change from column 1-54 as float variables
    cart_sample_sds[cart_sample_sds.columns[1:54]
                    ] = cart_sample_sds[cart_sample_sds.columns[1:54]].astype(float)
    cart_norm_sds[cart_norm_sds.columns[1:54]
                  ] = cart_norm_sds[cart_norm_sds.columns[1:54]].astype(float)
    cart_normrank_sds[cart_normrank_sds.columns[1:54]
                      ] = cart_normrank_sds[cart_normrank_sds.columns[1:54]].astype(float)
