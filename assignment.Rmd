...
```
We need to specify the path to save multiple plots
```{r}
destination <- "F:/Master-Thesis-DifferentialPrivacy/Figures/compare_plots.pdf"

pdf(file = destination)

# specify to save plots in 2*2 grid
par()
```
```{r}
# try with 3 vars firstly
selected_cols <- c("B5", "B6", "B7")
syn_select_vars <- syn_data_filtered[, selected_cols]
bindori_select_vars <- bindori_dataset_filtered[, selected_cols]
```
```{r}
syn_select_vars[paste("B", 5, sep = "")]
```

```{r}
for (i in 1:3) {
  compare(object = data.frame(Pdata = syn_select_vars[paste("B", i+4, sep = "")]),
          data = data.frame(Pdata = bindori_select_vars[paste("B", i+4, sep = "")]),
          vars =c(paste("B", i+4, sep = "")) , cont.na = NULL,
...
```{r}
head(syn_data_filtered2$B1b_x1)
```

```{r}
compare(object = data.frame(B1b_x13 = syn_data_filtered2["B1b_x13"]),
        data = data.frame(B1b_x13 = bindori_dataset_filtered2["B1b_x13"]),
        vars = c("B1b_x13"), cont.na = NULL,
