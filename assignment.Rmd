...
```{r}
table(bindori_dataset_filtered$E5)
```

```{r}
table(syn_data_filtered$E5)
```
## Step3: make all the vars aligned with each other
...
```
```{r}
compare(object = syn_data_filtered,
        data = bindori_dataset_filtered,
        vars = NULL, cont.na = NULL,
        msel = NULL, stat = "percents",
        utility.stats = c("pMSE", "S_pMSE"),
        cols = c("#1A3C5A","#4187BF"),
        plot = TRUE, table = FALSE)
...
```{r}
## S3 method for class 'data.frame'
utility.twoway <- utility.tables(object = data.frame(syn_data_filtered), 
                                 data = data.frame(bindori_dataset_filtered+),
                                 tables = "twoway",
                                 tab.stats = c("pMSE", "S_pMSE"), 
                                 plot.stat = "S_pMSE", plot = TRUE,  
                                 print.tabs = TRUE)
